...
### Changed
- **Bug Fixes**
- Improved security by implementing HTML sanitization across confirmation dialogs to prevent potential code injection vulnerabilities when displaying user and system-generated content.
- **Tests**
- Added comprehensive test coverage for XSS protection mechanisms in confirmation modals.
- <!-- review_stack_entry_start -->
- [![Review Change Stack](https://storage.
- <!-- review_stack_entry_end -->
- <!-- end of auto-generated comment: release notes by coderabbit.
- [ ] Malformed input raises (does not silent-coerce)
- [ ] Extreme size / depth handled or rejected explicitly
- [ ] Injection / unicode edges
- [ ] N/A — reason:
- # Floor checks
- [ ] `.
- [ ] Combined `--cov-branch` on `local_publication` ≥ 99%.
- [ ] Jest count not regressed.
- [ ] No tests deleted / skipped / xfailed-without-justification.
- [ ] No `--no-verify` on commit.
- # Risk
- <!-- Anything reviewer should look at carefully: cross-slice impact, perf, migrations, etc.
- <!-- This is an auto-generated comment: release notes by coderabbit.
- [ ] Keyboard navigation
- [ ] Screen-reader (`role`, `aria-live`, `aria-label`)
- [ ] Modified click (Cmd/Ctrl/middle-click preserves default)
- [ ] Paste / extreme input
- [ ] Disabled-state
- [ ] N/A — reason:
- [ ] Concurrent execution (uniqueness / lost-write)
- [ ] Real-Neo4j integration test (`@pytest.
- [ ] Transactional isolation
- [ ] N/A — reason:
- [ ] Success path
- [ ] Transient failure (retryable)
- [ ] Exhausted retry (final-failure tombstone)
- [ ] Cleanup-after-error (`finally:` preserves original)
- [ ] Idempotent re-run
- [ ] N/A — reason:
- [ ] Happy path
- [ ] Invalid input
- [ ] Auth-denied
- [ ] Cross-tenant
- [ ] Concurrent / idempotent submission
- [ ] N/A — reason:
- Adds a shared escapeHtml() helper and auto-escapes fields that should never contain HTML (title, button text, list items, and requireTextInput placeholder/requiredText) so call sites cannot accidentally introduce XSS via these fields.
- Fields that intentionally allow rich HTML (message, section.
- project-delete / scope-delete: replace brittle textContent-hack
- tenant-admin-group-detail: escape group name and member email
- base-delete-handler / delete-handler: escape entityName
- reliable-scope-unpublish / reliable-project-unpublish: escape scope/project/space names
- reliable-publisher/publish-mode-modal: escape object/space names
- Adds 10 regression tests that pipe real <img onerror> and <script> payloads through the modal and assert no execution and no injected DOM nodes, plus a positive test that <strong> in section.
- # Summary
- <!-- 1-3 sentences: what changed and why.
- # Test plan
- <!-- Which test types apply to this change? Tick the boxes for the layer(s) you touched.
- <!-- Reference: 0-docs/test-charter.

## [v8.8.67] - 2026-06-22

### Changed
- **Chores**
- Updated pinned project dependencies to newer patch versions to improve stability and security.
- Improved the dependency preparation process used for security auditing by normalizing certain requirement entries before audit generation.
- Expanded the security audit ignore list to reduce false positives for selected advisories.
- <!-- end of auto-generated comment: release notes by coderabbit.
- This PR edits `requirements.
...
- | `tornado` | 6.
- | `ujson` | 5.
- All bumps are the versions pip-audit itself recommends.
- The scheduled **pip-audit** workflow has been failing daily.
...
- Adds a project-root `CLAUDE.
- Acts as a baseline alongside the existing per-account `CLAUDE.
